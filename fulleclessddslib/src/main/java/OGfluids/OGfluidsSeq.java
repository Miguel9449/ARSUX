...
/**
* A sequence of OGfluids instances.
*/
public final class OGfluidsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OGfluidsSeq() {
        super(OGfluids.class);
    }

    public OGfluidsSeq (int initialMaximum) {
        super(OGfluids.class, initialMaximum);
    }

    public OGfluidsSeq (Collection elements) {
        super(OGfluids.class, elements);
    }

    public  OGfluids get(int index) { 
        return (OGfluids) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OGfluids.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OGfluids objects (beyond the original size of this object)
