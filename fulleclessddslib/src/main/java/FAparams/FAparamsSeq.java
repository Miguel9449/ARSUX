...
/**
* A sequence of FAparams instances.
*/
public final class FAparamsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FAparamsSeq() {
        super(FAparams.class);
    }

    public FAparamsSeq (int initialMaximum) {
        super(FAparams.class, initialMaximum);
    }

    public FAparamsSeq (Collection elements) {
        super(FAparams.class, elements);
    }

    public  FAparams get(int index) { 
        return (FAparams) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FAparams.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FAparams objects (beyond the original size of this object)
