...
/**
* A sequence of FAfluids instances.
*/
public final class FAfluidsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FAfluidsSeq() {
        super(FAfluids.class);
    }

    public FAfluidsSeq (int initialMaximum) {
        super(FAfluids.class, initialMaximum);
    }

    public FAfluidsSeq (Collection elements) {
        super(FAfluids.class, elements);
    }

    public  FAfluids get(int index) { 
        return (FAfluids) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FAfluids.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FAfluids objects (beyond the original size of this object)
