 ...
 * @author nextstep
 */
public class OGonModel extends CompNodeDataModel {
    ...
    public OGonDataReader reader;
    public OGonDataWriter writer;
    public OGonSeq dataSeq;
    public SampleInfo info;

    public DDS_Signal id; //@key
    public DDS_Signal state;
    public DDS_Signal rank;
    public DDS_Signal OGon; // 0


    public OGonModel(FragmentsModel fragmentsModel) {
        super(fragmentsModel);
        this.topicName = OGonTypeSupport.get_type_name();
        this.setCommStatus(topicName + " not active!");
        reader = fragmentsModel.oGonDataReader;
        writer = fragmentsModel.oGonDataWriter;
    ...
    public void initSignalSet() {
        root = DDS_Signal.createDDSSignal("OGA On", SignalType.rootSet);
        ...
        OGon = DDS_Signal.createDDSSignal("OGon", SignalType.command);

        root.getChildren().add(id);
        root.getChildren().add(state);
        root.getChildren().add(rank);
        root.getChildren().add(OGon);
    ...
    public void updateSignalSet() {
        ...
        if (topic != null) {
            ...
            OGon.setValue(new Object[]{topic.OGon});
        }
    ...
    public void initReader() {
        String typeName = OGonTypeSupport.get_type_name();
        //System.out.println("************************* TypeName " +typeName+"\n");
        OGonTypeSupport.register_type(participant, typeName);
        ...
        if (aTopic == null) {
            ...
            reader = (OGonDataReader) subscriber.create_datareader(
                    aTopic,
    ...
    public void initWriter() {
        try {
            ...
            writer = (OGonDataWriter) publisher.create_datawriter(
    ...
    public void updateEMeasures() {
        if(topic !=null) {
            ...
            OGon.updateeMeasure(new Double[]{topic.OGon}, Unit.ONE);
        }
    }
