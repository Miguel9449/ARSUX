...
public class WPuserTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "WPuser";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final WPuserTypeSupport _singleton
    = new WPuserTypeSupport();
    ...
    */
    public static WPuserTypeSupport get_instance() {
        return _singleton;
    }

    public static WPuserTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return WPuserTypeCode.VALUE;
    }
    ...
    * <code>this</code> is not a <code>WPuser</code>
    * type.
    */
    public Object copy_data(Object destination, Object source) {

        WPuser typedDst = (WPuser) destination;
        WPuser typedSrc = (WPuser) source;
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            dst.writeDouble(typedSrc.WWkgMin);

            dst.writeDouble(typedSrc.WWkgTrip);

            dst.writeDouble(typedSrc.WSkgMin);

            dst.writeDouble(typedSrc.WSkgTrip);

            dst.writeDouble(typedSrc.WSkgMax);
        }
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            WPuser typedDst = (WPuser) dst;
            typedDst.clear();      
            try{
                typedDst.id = src.readInt();
                typedDst.state = src.readInt();
                typedDst.rank = src.readInt();
                typedDst.WWkgMin = src.readDouble();
                typedDst.WWkgTrip = src.readDouble();
                typedDst.WSkgMin = src.readDouble();
                typedDst.WSkgTrip = src.readDouble();
                typedDst.WSkgMax = src.readDouble();
    ...
    */
    ...
    int mask) {
        return new WPuserDataWriter(native_writer, listener, mask, this);
    }
    ...
    int mask) {
        ...
        return new WPuserDataReader(native_reader, listener, mask, this);
    ...
    protected WPuserTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME,true,WPuserTypeCode.VALUE,WPuser.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected WPuserTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,WPuserTypeCode.VALUE,WPuser.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
