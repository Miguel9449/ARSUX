 ...
 * @author nextstep
 */
public class WPonModel extends CompNodeDataModel {

    public WPon topic;
    public Topic aTopic;
    public WPonDataReader reader;
    public WPonDataWriter writer;
    public WPonSeq dataSeq;
    public SampleInfo info;

    public DDS_Signal id; //@key
    public DDS_Signal state;
    public DDS_Signal rank;
    public DDS_Signal WPon; // 0


    public WPonModel(FragmentsModel fragmentsModel) {
        super(fragmentsModel);
        this.topicName = WPonTypeSupport.get_type_name();
    ...
    public void initSignalSet() {
        root = DDS_Signal.createDDSSignal("WPA On Command", SignalType.rootSet);
        ...
        WPon = DDS_Signal.createDDSSignal("WPon", SignalType.command);

        root.getChildren().add(id);
        root.getChildren().add(state);
        root.getChildren().add(rank);
        root.getChildren().add(WPon );
    ...
    public void updateSignalSet() {
        ...
        if (topic != null) {
            ...
            WPon.setValue(new Object[]{topic.WPon});
    ...
    public void initReader() {
        String typeName = WPonTypeSupport.get_type_name();
        //System.out.println("************************* TypeName " +typeName+"\n");
        WPonTypeSupport.register_type(fragmentsModel.participant, typeName);
        ...
        if (aTopic == null) {
            ...
            reader = (WPonDataReader) fragmentsModel.subscriber.create_datareader(
                    aTopic,
    ...
    public void initWriter() {
        try {
            ...
            writer = (WPonDataWriter) fragmentsModel.publisher.create_datawriter(
    ...
    public void updateEMeasures() {
        if(topic !=null) {
           ...
           WPon.updateeMeasure(new Double[]{topic.WPon}, Unit.ONE);
        }
    }
