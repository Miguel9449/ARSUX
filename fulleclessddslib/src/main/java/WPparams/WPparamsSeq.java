...
/**
* A sequence of WPparams instances.
*/
public final class WPparamsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WPparamsSeq() {
        super(WPparams.class);
    }

    public WPparamsSeq (int initialMaximum) {
        super(WPparams.class, initialMaximum);
    }

    public WPparamsSeq (Collection elements) {
        super(WPparams.class, elements);
    }

    public WPparams get(int index) {
        return (WPparams) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WPparams.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WPparams objects (beyond the original size of this object)
