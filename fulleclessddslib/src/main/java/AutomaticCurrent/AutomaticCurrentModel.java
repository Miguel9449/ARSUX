 ...
 * @author nextstep
 */
public class AutomaticCurrentModel extends CompNodeDataModel {

    public AutomaticCurrent topic;
    public Topic aTopic;
    public DomainParticipant participant;
    public Subscriber subscriber;
    public Publisher publisher;
    public AutomaticCurrentDataReader reader;
    public AutomaticCurrentDataWriter writer;
    public AutomaticCurrentSeq dataSeq;
    public SampleInfo info;


    public DDS_Signal id; //@key
    public DDS_Signal current; // 0



    public AutomaticCurrentModel(FragmentsModel fragmentsModel) {
        super(fragmentsModel);
        this.topicName = AutomaticCurrentTypeSupport.get_type_name();
        this.setCommStatus(topicName + " not active!");
        reader = fragmentsModel.automaticCurrentDataReader;
        writer = fragmentsModel.automaticCurrentDataWriter;
    ...
    public void initSignalSet() {
        root = DDS_Signal.createDDSSignal("Automatic CurrentTelemetry", SignalType.rootSet);
        current=DDS_Signal.createDDSSignal("Automatic Current", SignalType.current_sensor);

        //sample: co2SetLabel = DDS_Signal.createDDSSignal("CO2 Sensors", SignalType.information);
        sample: root.getChildren().add(current);
    ...
    public void initReader() {
        String typeName = AutomaticCurrentTypeSupport.get_type_name();
        //System.out.println("************************* TypeName " +typeName+"\n");
        AutomaticCurrentTypeSupport.register_type(participant, typeName);
        ...
        if (aTopic == null) {
            ...
            reader = (AutomaticCurrentDataReader) subscriber.create_datareader(
                    aTopic,
    ...
    public void initWriter() {
        try {
            ...
            writer = (AutomaticCurrentDataWriter) publisher.create_datawriter(
    ...
    public void updateTopicData() {
        ...
        if (reader != null) {
            ...
            try {
                dataSeq = new AutomaticCurrentSeq();
...
//   System.err.println("***********************************in updatetopicdata, the infoSeq.size is "+ dataSeq.size()+"\n");
                for (int i = 0; i < dataSeq.size(); ++i) {
                    ...
                    if (info.valid_data) {
                        topic = ((AutomaticCurrentSeq) dataSeq).get(i);
                        if(topic == null)  {
