...
/**
* A sequence of WPfluids instances.
*/
public final class WPfluidsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WPfluidsSeq() {
        super(WPfluids.class);
    }

    public WPfluidsSeq (int initialMaximum) {
        super(WPfluids.class, initialMaximum);
    }

    public WPfluidsSeq (Collection elements) {
        super(WPfluids.class, elements);
    }

    public WPfluids get(int index) {
        return (WPfluids) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WPfluids.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WPfluids objects (beyond the original size of this object)
