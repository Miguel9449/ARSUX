...
/**
* A sequence of UWMSuser instances.
*/
public final class UWMSuserSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public UWMSuserSeq() {
        super(UWMSuser.class);
    }

    public UWMSuserSeq (int initialMaximum) {
        super(UWMSuser.class, initialMaximum);
    }

    public UWMSuserSeq (Collection elements) {
        super(UWMSuser.class, elements);
    }

    public  UWMSuser get(int index) { 
        return (UWMSuser) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, UWMSuser.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' UWMSuser objects (beyond the original size of this object)
